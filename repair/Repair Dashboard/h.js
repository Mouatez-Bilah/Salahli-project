...
document.addEventListener('DOMContentLoaded', function() {
    ...
    async function getRepairProfile() {
        try {
            const token = localStorage.getItem("Rtoken");
            const response = await axios.get('http://127.0.0.1:8000/api/repair-profile/me/', {
                headers: {
            ...
            if(response.status >= 200 && response.status < 300) {
                localStorage.setItem("repair", JSON.stringify(response.data));
                console.log(response.data);
                
                // Update username display after successful profile fetch
                const username = localStorage.getItem("Rusername");
